...
setwd("C:\\Users\\user\\Documents\\bigdata\\9_Microsoft R\\20190627")
...
remoteLogin("http://mlso.southeastasia.cloudapp.azure.com:12800", session = TRUE, diff = TRUE, commandline = TRUE)
...
flightDataSampleXDF <- rxImport(inData = "C:\\TestData\\2000.csv", 
                                outFile = "C:\\TestData\\Sample.xdf", overwrite = TRUE, append = "none", colClasses = flightDataColumns,
                                transforms = list(
...
flightDataCsvFolder <- "C:\\TestData\\Data"
...
conString <- "Driver=SQL Server;Server=mysvn.database.windows.net;Database=AirlineData;Uid=admin2019;Pwd=Pa200rd.56$$"
...
delayFactor <- expression(list(Delay = cut(Delay, breaks = c(0, 1, 30, 60, 120, 180, 181), 
                                           labels = c("No delay", "Up to 30 mins", "30 mins - 1 hour", 
                                                      "1 hour to 2 hours", "2 hours to 3 hours", "More than 3 hours"))))
