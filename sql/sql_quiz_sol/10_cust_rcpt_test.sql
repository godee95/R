...
2. 현재 살아있는 계좌수가 두 개 이상이고 수신잔액의 총합이 50만원 이상인 
	고객의 주민번호, 이름, 휴대전화, 
...
/*
select cp.ssn, mobile_no, count(acct_no) as num_acct 
from cust_party cp left outer join rcpt_acct ra on cp.ssn = ra.ssn
...
where clcn_dt is null group by 1) tmp2
...
select * from rcpt_acct rc
left outer join cust_party cp
on rc.ssn = cp.ssn;
