...
select * from purchase_tran where purchase_amt between 1000000 and 5000000;
select * from purchase_tran where purchase_cnt > last_cnt;
select (purchase_amt/puchase_cnt) as 평균구매금액 from puchase_tran;
select (purchase_amt/puchase_cnt) as 올해평균구매금액, (last_amt/last_cnt) as 작년평균구매금액 from puchase_tran;
