...
select gender, avg(round(datediff(now(), birth_dt)/365)) as age from clerk group by gender;
select def_nm, avg(round(datediff(now(), birth_dt)/365)) as age from clerk group by def_nm order by age limit 1;
