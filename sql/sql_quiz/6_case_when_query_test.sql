...
select * from stud_score;
select * from stud_score where math_score is null or eng_score is null or phil_score is null or music_socre is null;
...
select student_id,(coalesce(math_score,0)+coalesce(eng_score,0)+coalesce(phil_score,0)+coalesce(music_score,0)) as total_sum from stud_score order by total_score desc limit 1;
...
select student_id, sum(stddev(math_score)+stddev(eng_score)+stddev(phil_score)+stddev(music_score)) as substd from stud_score;
select var_samp(maht_score), var_samp(eng_score), var_samp(phil_score), var_samp(music_score) from stud_score;
...
when job='manager' and eng_score > 90 then current_sal*1.08
when job='manager' and eng_score <= 90 then current_sal*1.03
else current_sal end) as 임금인상비용
from staff_sal;
